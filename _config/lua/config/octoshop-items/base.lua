...
octoshop.items['time_25h'] = {
	cat = L.octoshop_buns,
	order = 3,
	name = "+25 часов к общему времени",
	desc = "Моментально добавляет 25 часов к вашему общему игровому времени",
	attributes = {
		{L.octoshop_what, octolib.icons.silk16('clock_add'), "Добавление игрового времени"},
		{L.octoshop_where, octolib.icons.silk16('tick'), "Мгновенное применение"},
		{L.octoshop_validity, octolib.icons.silk16('infinity'), "Постоянное действие"},
	},
	price = 300,
	icon = 'octoteam/icons/clock_plus.png',
	CanBuy = true,
	OnGiven = function(self)
		if not self:GetData('usesLeft') then self:SetData('usesLeft', 1) end
	end,
	...
	Use = function(self)
		local ply = self:GetOwner()
		local addedTime = 25 * 60 * 60
		--добавка только к ВСЕМУ НАИГРАННОМУ времени. Кодер hesus3332 еблан
		ply:SetTimeTotal(ply:GetNetVar('pt.total', 0) + addedTime)
		
		self:SetData('usesLeft', 0)
		ply:osNetInv()
		ply:Notify('hint', 'Вам успешно добавлено 25 часов общего игрового времени!')
	end,
}
...
octoshop.items['build'] = {
    ...
    CanTrade = hasUses,
    CanUse = function(self)
        return self:GetData('usesLeft') > 0 and not self:GetOwner():GetNetVar('os_build')
    end,
    OnGiven = function(self)
        if not self:GetData('usesLeft') then
            self:SetData('usesLeft', 1)
        end
        ...
        if self:GetData('active') then
            ...
            -- Получаем текущий лимит пропов
            local currentLimit = ply:GetNetVar('propLimit', 512) -- По умолчанию 512
            -- Сохраняем оригинальный лимит, если ещё не сохранили
            if not ply:GetNetVar('originalPropLimit') then
                ply:SetNetVar('originalPropLimit', currentLimit)
            end

            -- Устанавливаем удвоенный лимит
            ply:SetNetVar('propLimit', currentLimit * 2)

            -- Меняем цвет физпушки, если он не стандартный
    ...
    OnTaken = function(self)
        if self:GetData('active') then
            ...
            -- Восстанавливаем оригинальный лимит пропов
            local originalLimit = ply:GetNetVar('originalPropLimit')
            if originalLimit then
                ply:SetNetVar('propLimit', originalLimit)
                ply:SetNetVar('originalPropLimit', nil) -- Очищаем
            end

            -- Восстанавливаем стандартный цвет физпушки
