octogui.scoreboard = octogui.scoreboard or {}

if IsValid(octogui.scoreboard) then
	octogui.scoreboard:Remove()
end

timer.Simple(0, function()
	local maxH = ScrH() - 80
	local p = vgui.Create 'DFrame'
	octogui.scoreboard = p

	local bgCol

	function p:OnShow()
		bgCol = table.Copy(CFG.skinColors.bg)
	end
	...
	p.canvas.VBar.Paint = function(_, w, h)
		draw.RoundedBox(4, w / 2 - 3, 18, 6, h - 36, CFG.skinColors.bg)
	end
	...
	local function paintButton(self, w, h)
		...
		if al > 0 then
			al = ease(al, 0, 1, 1)
			local tw = surface.GetTextSize(ply:Name())
	...
	local function thinkButton(self)
		...
		-- fines
		local fines = dbgPolice.fines.get(self.ply)
		if self.fine then
			local oldFinesAmount = self.fine[3]
			local finesAmount = table.Count(fines)

			if not LocalPlayer():isCP() and self.ply ~= LocalPlayer() then
				self.fine[2]:Remove()
				self.fine = nil
				return
			end

			if not dbgPolice.fines.has(self.ply) then
				self.fine[2]:Remove()
				self.fine = nil
				return
			end

			if oldFinesAmount ~= finesAmount then
				self.fine[2]:Remove()
				self.fine = nil
				return
			end

		elseif dbgPolice.fines.has(self.ply) then
			if not LocalPlayer():isCP() and self.ply ~= LocalPlayer() then
				return
			end
			...
			w:SetWide(24)
			w:SetCursor('hand')
			w.icon = Material(octolib.icons.silk16('zone_money'))
			w.Paint = paintTag

			local finesAmount = table.Count(fines)
			local finesKeys = table.GetKeys(fines)

			if finesAmount == 1 then
				w:AddHint(('Штраф на %s по причине: %s'):format(DarkRP.formatMoney(fines[finesKeys[1]].price), fines[finesKeys[1]].reason))
			else
				w:AddHint(('Штрафы: %s неоплаченных'):format(finesAmount))
			end

			w.DoClick = function(w)
				self:DoClick()
			end
			self.fine = {dbgPolice.fines.get(self.ply), w, finesAmount}
		end
	end
	...
	p.UpdatePlayerList = function(self)
		...
		local hiddenPlayerNum = 0
		for _, ply in ipairs(plys) do
			if hook.Run('dbg-score.hidePlayer', ply) == true then
				hiddenPlayerNum = hiddenPlayerNum + 1
				continue
			end
				...
				if ply ~= LocalPlayer() then
					...
					if FPP.Buddies[steamID] ~= nil then
						menu:AddOption(L.remove_friend, function()
							for _, acessType in pairs(types) do
		...
		self.hiddenPlayerNum = hiddenPlayerNum
	...
	p.Think = function(self)
		if not self.AddHint then return end
		...
		if LocalPlayer():IsAdmin() and not IsValid(self.adminBut1) then
			...
			function b3:DoClick()
				octogui.esc.OpenURL('https://octothorp.team/logs')
			end
	...
	function GAMEMODE:ScoreboardShow()
		...
		local plys = player.GetAll()
		for _, ply in ipairs(plys) do
			if not hook.Run('dbg-score.hidePlayer', ply) then continue end
			hiddenPlayerNum = hiddenPlayerNum + 1
		end

		if hiddenPlayerNum ~= p.hiddenPlayerNum then
			p:UpdatePlayerList()
		end
