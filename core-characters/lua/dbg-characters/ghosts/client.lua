...
File Path:   addons/core-characters/lua/dbg-characters/ghosts/client.lua
...
hook.Add('PostDrawHUD', 'dbg-characters.ghosts', function()
	...
	local imDead, spTime, ghTime = ply:IsGhost(), ply:GetNetVar('_SpawnTime', 0), ply:GetNetVar('_GhostTime', 0)
	...
	if not imDead then
		...
		if not ply:Alive() then
			local fadein = math.Clamp((ct - spTime + dbgChars.ghosts.config.spawnTime) / dbgChars.ghosts.config.fadeOutDeath, 0, 1)
			local al = 255 * fadein
	...
	elseif ply:GetNetVar('launcherActivated') then
		...
		if ghTime and ct < ghTime - 0.5 and getRagdoll(ply) then
			local fadein = math.Clamp((ghTime - ct - 0.75) / dbgChars.ghosts.config.ghostTime, 0, 1)
		...
		local fadeout = math.Clamp((spTime - ct) / dbgChars.ghosts.config.fadeOutGhost, 0, 1)
...
local function drawDeathText(x, y)
	if LocalPlayer():GetNetVar('_GhostTime') > CurTime() then
		return 0
	end

	draw.SimpleText(
		string.ToMinutesSeconds(math.max(LocalPlayer():GetNetVar('_SpawnTime', 0) - CurTime(), 0)),
		'DeathScreen',
		x,
		y,
		clrGray,
...
local function showTimer(show)
	if show then
		octolib.bottomMessages.add('ghost-timer', {
			paint = drawDeathText,
			time = LocalPlayer():GetNetVar('_SpawnTime', 0) - CurTime(),
		})
	else
...
hook.Add('octolib.netVarUpdate', 'dbg-characters.ghosts', function(index, key, value)
	if key == 'Ghost' and index == LocalPlayer():EntIndex() then
