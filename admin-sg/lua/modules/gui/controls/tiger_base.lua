...
function panel:Init()
	...
	self.copyright:SetToolTip("Click here to visit the ServerGuard website!")
...
function panel:SetUpdateNotification(bToggled)
	...
	if (bToggled) then
		...
		self.updatePanel:SetToolTip("Version " .. serverguard.GetLatestVersion() .. " of ServerGuard is now available!\nIt is recommended to update this server's version of ServerGuard.");
	else
		self.updatePanel:SetCursor("none");
		self.updatePanel:SetToolTip();
	end;
		...
		function base:Think()
			...
			if (!hasPermission) then
				if (!self.icon:GetDisabled()) then
					...
					self.icon:SetColor(theme.tiger_base_section_icon)
					self.icon:SetToolTip("You do not have permission to view this!")
