...
-- Сис-ма Перевода денежных средств
--[[
	local function transfer()
	local text, tgtName, check = '', ''
	local f = vgui.Create 'DFrame'
	f:SetSize(400, 135) -- 135, 175
	f:SetTitle('Перевод денежных средств')
	f:Center()
	f:MakePopup()
	f:SetBackgroundBlur(true)
	...
	b:SetEnabled(false)
	function b:DoClick()
		netstream.Start('chat', ('/transfer "%s" %s'):format(tgtName, text))
		f:Remove()
	end
	...
	e:DockMargin(5, 5, 5, 10)
	e:SetUpdateOnType(true)
	e:SetPlaceholderText('Введите сумму')
	e.PaintOffset = 5
	function e:OnValueChange(val)
		text = val
		check()
...
-- 	e:DockMargin(5, 5, 5, 10)
-- 	e:SetUpdateOnType(true)
-- 	e:SetPlaceholderText('Введите сообщение')
-- 	e.PaintOffset = 5
-- 	function e:OnValueChange(val)
-- 		text = val
-- 		check()
...
netstream.Hook('dbg-phone.open', function(center, ent)
	...
	local menu = DermaMenu()
        ...
        if not ent then
        	menu:AddOption('Позвонить', function()
			...
			end):SetIcon(octolib.icons.silk16('phone_sound'))
        end
	    menu:AddOption('Перевести деньги', transfer):SetIcon(octolib.icons.silk16('money'))
...
local phoneAnim = {
    calling = {
        default = {
        	Bip01_L_Hand = { ang = Angle(15, 15, 10) },
        	Bip01_L_Forearm = { ang = Angle(0, -125, -20) },
        	Bip01_L_Clavicle = { ang = Angle(0, 0, 0) },
        	Bip01_L_UpperArm = { ang = Angle(-30, -70, -10) },
...
hook.Add('UpdateAnimation', 'dbg-phone.UpdateAnimation', function(ply, vel)
	...
	ply.PhoneAnimWeight = math.Approach(
		...
		((ply:IsUsingPhone() or ply:GetNetVar('onCall') or ply:GetNetVar('isCalling')) and ply:OnGround()) and (1 - vel:LengthSqr() / 50000) or 0,
		FrameTime() * 5
	)
...
function phone.getProperAnim(ply)
    ...
    if ply:GetNetVar('onCall') or ply:GetNetVar('isCalling') then
    	return phoneAnim.calling.default
...
local enabled = false
octogui.cmenu.registerItem('rp', 'showPhone', {
	text = 'Взять телефон',
	icon = octolib.icons.silk16('phone_sound'),
	action = function()
    	netstream.Start('dbg-phone.typingSMS')
        if enabled then
            netstream.Start('dbg-phone.updateTypeStatus', false)
        else
            netstream.Start('dbg-phone.updateTypeStatus', true)
