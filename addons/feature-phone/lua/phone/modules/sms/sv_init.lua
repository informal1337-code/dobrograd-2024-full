...
local function sendSMS(sender, recipientName, message)
    ...
    netstream.Start(recipient, 'dbgPhone.receiveSMS', sender:Name(), message)
...
netstream.Hook('dbg-phone.updateTypeStatus', function(ply, status)
    if not canUsePhone(ply) then return end
    ply:SetNetVar("UsingPhone", status)
end)
