...
function ENT:ExitPassenger( ply, instant )
	...
	if ply.exiting then return end

	ply.exiting = true
	ply:DelayedAction('exitCar', "Выход из машины", {
		time = instant and 0 or 1.5,
		check = function()
			return IsValid(ply) and IsValid(self) and ply:InVehicle() and ply:GetVehicle():GetParent() == self
		end,
		succ = function()
			if IsValid(ply) then
				ply:ExitVehicle()
...
hook.Add('PlayerLeaveVehicle', 'simfphys-delay.fix', function(ply, veh) 
	ply.entering = nil 
	ply.exiting = nil
end)
