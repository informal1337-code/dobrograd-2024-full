...
local tabs = {
	[1] = {"Start","#start",(Material("stormfox2/hud/menu/dashboard.png")),function(board)
		...
		-- FPS
			fps = StormFox2.Menu.FPSRing()
			dash:Add(fps)
		-- Quality
			qu = StormFox2.Menu.QualityRing()
			dash:Add(qu)
		-- Support GPU
			sup = StormFox2.Menu.SupportRing()
			dash:Add(sup)
		-- MThread
			mth = StormFox2.Menu.MThreadRing()
			dash:Add(mth)
		...
		l:Dock(TOP)
		l:DockMargin(16, 0, 0, 0)
		l:SetText(language.GetPhrase("#sf_" .. "quality_target.desc"))
		l:SetMultiline(true)
		l:SetWrap(true)
		l:SetAutoStretchVertical(true)
...
function StormFox2.Menu.FPSRing()
	local fps = vgui.Create 'SF_Setting_Ring'
	fps:SetText('FPS: ')
	fps:SetSize(90, 90)
	function fps:Think()
		...
		self:SetText('FPS: ' .. math.floor(avgFPS))
	end
	fps:SetColor(colors.g)
	fps:SetTooltip('Соотношение текущего FPS к желаемому')
	return fps
end

function StormFox2.Menu.QualityRing()
	local qu = vgui.Create 'SF_Setting_Ring'
	qu:SetText('Эффекты')
	qu:SetSize(90, 90)
	function qu:Think()
		if (self.u_t or 0) > SysTime() then return end
		...
		local q = StormFox2.Client.GetQualityNumber()
		local f = q / max_q
		self:SetValue(f)
		self:SetText(('Эффекты\n%d%%'):format(math.floor(f * 100)))
	end
	qu:SetColor(colors.g)
	return qu
end

function StormFox2.Menu.SupportRing()
	local sup = vgui.Create 'SF_Setting_Ring'
	sup:SetText('Поддержка')
	sup:SetSize(90, 90)
	function sup:Think()
		if (self.u_t or 0) > SysTime() then return end
		if not system.HasFocus() then return end
		self.u_t = SysTime() + 5
		local t = {
			{ render.SupportsPixelShaders_1_4(), '#problem.no_ps14' },
			{ render.SupportsVertexShaders_2_0(), '#problem.no_vs20' },
			{ render.SupportsPixelShaders_2_0(), '#problem.no_ps20' },
			{ render.SupportsHDR(), '#problem.no_hdr' },
		}
		local s = {}
		for _, v in ipairs(t) do
			if not v[1] then s[#s + 1] = v[2] end
		end
		self:SetTooltip(s[1] and table.concat(s, '\n') or 'Тут все в порядке!')
		local ok = #t - #s
		self:SetValue(ok / #t)
		self:SetText(('Поддержка\n%d/%d'):format(ok, #t))
	end
	sup:SetColor(colors.g)
	return sup
end

local mThreadBestSettings = {
...
function StormFox2.Menu.MThreadRing()
	local mth = vgui.Create 'SF_Setting_Ring'
	mth:SetText('MThread')
	mth:SetSize(90, 90)
	function mth:Think()
		if (self.u_t or 0) > SysTime() then return end
		...
		local f = ok / n
		self:SetValue(f)
		self:SetText(('MThread\n%d/%d'):format(ok, n))
		local val
		if f < 1 then
			val = ('Следующие настройки могут увеличить производительность:\n\n%s\n\nВНИМАНИЕ:\nЭти настройки могут приводить к вылетам на некоторых старых процессорах AMD!'):format(table.concat(s, '\n'))
		else val = 'Тут все в порядке!' end
		self:SetTooltip(val)
	end
	mth:SetColor(colors.g)
	return mth
end

function StormFox2.Menu.FPSTarget()
	local FPSTarget = vgui.Create 'SF_Setting'
		...
		function slider:Think()
			if self:IsDown() then
				...
				if self._OvR < 40 then
					AddTip(self, 'Играть с низким FPS может быть некомфортно!')
				else
					RemoveTip(self)
				end
