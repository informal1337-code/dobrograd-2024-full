...
duplicator.Allow('prop_dynamic')
duplicator.Allow('prop_dynamic_override')
...
if(SERVER)then
	...
	function TOOL:LeftClick( trace )
		...
		if(tobool(ply:GetInfo("advdupe2_original_origin")))then
			origin = Vector(ply.AdvDupe2.HeadEnt.Pos)
		end
		local x, y, z = ply:GetInfo('advdupe2_original_offset_x'), ply:GetInfo('advdupe2_original_offset_y'), ply:GetInfo('advdupe2_original_offset_z')
		if (x ~= 0 or y ~= 0 or z ~= 0) and origin then
			origin:Add(Vector(x, y, z))
		end
		AdvDupe2.InitPastingQueue(ply, ply.AdvDupe2.Position, ply.AdvDupe2.Angle, origin, tobool(ply:GetInfo("advdupe2_paste_constraints")), tobool(ply:GetInfo("advdupe2_paste_parents")), tobool(ply:GetInfo("advdupe2_paste_disparents")),tobool(ply:GetInfo("advdupe2_paste_protectoveride")),tobool(ply:GetInfo("advdupe2_remove_physics")), tobool(ply:GetInfo("advdupe2_make_perma")))
		return true
	end
	...
	function AdvDupe2.FinishPasting(Player, Paste)
		...
		if Player.oldKeypads then
			Player:Notify("В твоем дубликате присутствовали кейпады, которые были созданы до одного из обновлений дубликатора, в результате чего больше не работают. Убедись, что все кейпады работают корректно, и пересохрани дубликат.")
			Player.oldKeypads = nil
		end
...
if(CLIENT)then
	...
	CreateClientConVar("advdupe2_remove_physics", 1, false, true)
	CreateClientConVar("advdupe2_make_perma", 0, false, true)
	CreateClientConVar("advdupe2_original_origin", 0, false, true)
	CreateClientConVar("advdupe2_original_offset_x", 0, false, true)
	CreateClientConVar("advdupe2_original_offset_y", 0, false, true)
	CreateClientConVar("advdupe2_original_offset_z", 0, false, true)
	...
	local function BuildCPanel(CPanel)
		...
		Check:SetText( "Сделать пропы статичными" )
		Check:SetDark(true)
		Check:SetConVar( "advdupe2_remove_physics" )
		Check:SetValue( 1 )
		Check:SetToolTip("Делает все пропы статичными. Это убирает возможность их разморозить, но решает проблемы со светом и стабильностью")
		CPanel:AddItem(Check)

		if LocalPlayer():query(L.permissions_permaprops) then
			Check = vgui.Create("DCheckBoxLabel")
			Check:SetText( "Сделать постоянным" )
			Check:SetDark(true)
			Check:SetConVar( "advdupe2_make_perma" )
			Check:SetValue(0)
			CPanel:AddItem(Check)
		end
		...
		local PPos = vgui.Create("DPanel")
		PPos:SetTall(43)
		PPos:SetPaintBackground(false)
		for _, v in ipairs({"x", "y", "z"}) do
			local cont = PPos:Add("DPanel")
			cont:Dock(LEFT)
			cont:DockMargin(0, 0, 5, 0)
			cont:SetWide(85)
			cont:SetPaintBackground(false)

			local cv = GetConVar("advdupe2_original_offset_" .. v)
			local nw, title = octolib.numberWang(cont, "Смещение " .. utf8.upper(v), cv:GetFloat(), -5000, 5000)
			title:DockMargin(0, 0, 0, 0)
			function nw:OnValueChanged(val)
				cv:SetFloat(val)
