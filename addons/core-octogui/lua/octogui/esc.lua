...
surface.CreateFont('octogui.esc.large', {
	font = 'Calibri',
	extended = true,
	size = 42,
	weight = 350,
...
timer.Simple(0, function()
	...
	if IsValid(octogui.esc.panel) then octogui.esc.panel:Remove() end

	local esc = vgui.Create 'DPanel'
	octogui.esc.panel = esc
	esc:SetSize(pw, ph)
	esc:SetPaintBackground(false)
	esc:MakePopup()
	esc:Center()
	esc.al = 0

	hook.Add('Think', 'octogui.esc', function()
		if gui.IsGameUIVisible() and not forceMenu then
			if (gui.IsConsoleVisible() and input.IsKeyDown(KEY_BACKQUOTE)) or showConsole then
				showConsole = true
				return
			end
	...
	hook.Add('RenderScreenspaceEffects', 'octogui.esc', function()
		local state = esc.al
	...
	octogui.esc.OpenURL = gui.OpenURL
	...
	for i, v in ipairs({
		{'Продолжить', function() esc:Toggle(false) end},

		{'spacer'},

		{'Сайт', function() octogui.esc.OpenURL('https://octo.gg/site') end},
		{'Дискорд', function() octogui.esc.OpenURL('https://octo.gg/discord') end},
		{'Форум', function() octogui.esc.OpenURL('https://octo.gg/forum') end},
		{'Вики', function() octogui.esc.OpenURL('https://octo.gg/wiki') end},
		{'ВКонтакте', function() octogui.esc.OpenURL('https://octo.gg/vk') end},
		{'Обратная связь', function() octogui.esc.OpenURL('https://octo.gg/feedback') end},
		...
		{'Отключиться', function()
			...
			if octoinv and octoinv.invs and octoinv.invs[eID] and octoinv.invs[eID]._hand and octoinv.invs[eID]._hand.items[1] then
	...
	local scrollPanel = esc:Add 'DScrollPanel'
	...
	esc.RefreshServers = function(self)
		if not octoservices then return end
		...
		octoservices:get('/servers/status'):Then(function(res)
			...
			for _, group in ipairs(res.data or {}) do
				...
				if not IsValid(srvPanel) then
					...
					name:Dock(TOP)
					name:SetTall(35)
					name:SetFont('octogui.esc.large')
