...
File Path:   addons/core-octogui/lua/octogui/cmenu/client.lua
...
function octogui.cmenu.create()
	...
	local function handleItem(item, m)
		if item.spacer then m:AddSpacer()
			return
		end

		local text
		if isfunction(item.text) then
			text = item.text(ply)
		...
		elseif item.options then
			local options = isfunction(item.options) and item.options(ply) or item.options
			if not options or #options == 0 then
				return
			end

			subMenu, option = m:AddSubMenu(text)
			for _, subOption in ipairs(options) do
				if not subOption.check or subOption.check(ply) then
					handleItem(subOption, subMenu)
				end
			end
		else
			...
			if not action then
				...
				if item.url then
					action = function() octogui.esc.OpenURL(item.url) end
				end
