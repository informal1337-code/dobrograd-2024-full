-- "addons\\core-octolib\\lua\\octolib\\modules\\render\\vgui\\octolib_rendermodel.lua"
...
concommand.Add('octolib_rendermodel', function()
	...
	local rebuild = octolib.func.debounce(function()
		octolib.renderModel.clear(dest, renderSettings)
		octolib.renderModel.queueRender(dest, renderSettings)
	end, 0.1)

	local eModel = octolib.textEntry(scrollPanel, 'Модель')
	...
	local eColor = octolib.colorPicker(scrollPanel, 'Цвет', false, false)
	function eColor:ValueChanged(val)
		dest.color = val
		rebuild()
	end
	eColor:SetColor(dest.color)

	local sWidth = octolib.slider(scrollPanel, 'Ширина', 16, 512, 0)
	...
	local sHeight = octolib.slider(scrollPanel, 'Высота', 16, 512, 0)
	...
	local sFov = octolib.slider(scrollPanel, 'FOV', 10, 90, 1)
	...
	local sFovMod = octolib.slider(scrollPanel, 'Множитель FOV', 0.2, 2, 2)
	...
	local sPitch = octolib.slider(scrollPanel, 'Угол: Pitch', -180, 180, 0)
	...
	local sYaw = octolib.slider(scrollPanel, 'Угол: Yaw', -180, 180, 0)
	...
	local sRoll = octolib.slider(scrollPanel, 'Угол: Roll', -180, 180, 0)
	...
	local sX = octolib.slider(scrollPanel, 'Позиция: X', -500, 500, 1)
	...
	local sY = octolib.slider(scrollPanel, 'Позиция: Y', -500, 500, 1)
	...
	local sZ = octolib.slider(scrollPanel, 'Позиция: Z', -500, 500, 1)
