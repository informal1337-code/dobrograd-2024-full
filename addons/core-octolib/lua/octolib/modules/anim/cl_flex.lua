...
File Path:   addons/core-octolib/lua/octolib/modules/anim/cl_flex.lua
...
function ent:ApplyFlex(flexes, time, keepExisting)
	...
	for i = 0, self:GetFlexNum() - 1 do
		...
		if not keepExisting then tgt[i] = 0 end
	end
	...
	for flexID, weight in pairs(tgt) do
		if old[flexID] == weight or not tgt[flexID] then
			old[flexID] = nil
			tgt[flexID] = nil
		end
	end
...
octolib.dataEditor.register('faceposes', {
	...
	validate = function(row)
		if not isstring(row.name) and string.Trim(row.name) ~= '' and istable(row.flexes) then
			return false
		end
	end,
})
