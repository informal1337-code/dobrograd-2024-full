...
hook.Add("InitPostEntity", "e2lib", function()
    -- Проверяем доступность CPPI
    if SERVER and CPPI then
        local registry = debug.getregistry()

        -- Безопасная проверка наличия Player и CPPIGetFriends
        if registry and registry.Player and registry.Player.CPPIGetFriends then
        ...
        -- Безопасная проверка наличия Entity и CPPIGetOwner
        if registry and registry.Entity and registry.Entity.CPPIGetOwner then
