...
local minTimeToBuild =  10 * 60 -- 10 минут епт
local newbieMsg = 'У тебя недостаточно наигранного времени для того, чтобы начать строить. Советуем уделить первые 10 минут игры ознакомлению с механиками сервера!'
local function canSpawn(ply)
	...
	if ply:TriggerCooldown('lookbook', 10) then
		local request = {{
			name = 'Стиль построек Восточного Побережья',
			desc = 'Мы заинтересованы в том, чтобы все постройки в городе выглядели естественно и вписывались в общую картину. Похоже, ты собираешься внести свой вклад в эту картину.',
		}}
		if CFG.dev or ply:GetTimeTotal() >= minTimeToBuild then
			request[#request + 1] = {
				desc = 'Если администраторы посчитают твою постройку нарушающей общую стилистику Восточного Побережья, они в уведомлении попросят тебя исправить что-то в ней или перестроить вовсе. Если эти просьбы будут проигнорированы, им придется удалить постройку.\n\nОтметь ниже, когда ознакомишься с лукбуком, осознаешь важность поддержания общей стилистики Стрэйфилда и будешь готов украшать город своей постройкой:',
				type = 'check',
				txt = 'Вперед строить!',
				required = true,
			}
