...
local function loadStations(lst, curOffset, data)
	...
	if #data == 25 then
		...
		function octoradio.btnNext:DoClick()
			lst:Clear()
			netstream.Request('octoradio.getByPlace', {'Strayfield MI', 'United States', curOffset + 25}):Then(function(data)
	...
	if curOffset > 0 then
		...
		function octoradio.btnPrev:DoClick()
			lst:Clear()
			netstream.Request('octoradio.getByPlace', {'Strayfield MI', 'United States', math.max(curOffset - 25, 0)}):Then(function(data)
...
function octoradio.populateDobrograd(pan)
	...
	netstream.Request('octoradio.getByPlace', {'Strayfield MI', 'United States', 0}):Then(function(data)
