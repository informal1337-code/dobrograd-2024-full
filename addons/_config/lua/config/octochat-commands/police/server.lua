...
octochat.registerCommand('/pbroadcast', {
	cooldown = 30,
	log = true,
	execute = function(ply, txt)
		if txt == '' then return 'Формат: /pbroadcast Текст сообщения' end
		if not ply:isChief() then
			return 'Только лейтенант полиции может использовать эту команду'
		end
		for _, v in ipairs(player.GetAll()) do
			octochat.talkTo(v, octochat.textColors.rp, '[Полиция] ', color_white, txt)
			v:EmitSound('ambient/announcements/security_alert.wav', 75, 100, 0.7)
		end
		ply:Notify('warning', 'Сообщение передано всем гражданам')
		hook.Run('OnPoliceBroadcast', ply, txt)
