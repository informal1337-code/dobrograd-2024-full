...
octochat.registerCommand('transfer', {
    phone = true,
    check = function(ply, targetName, amount)
        if not targetName or not amount then return false end
        amount = tonumber(amount)
        if not amount or amount <= 0 then return false, "Укажите положительную сумму" end
        
        local target = octolib.findPlayer(targetName)
        if not IsValid(target) then return false, "Игрок не найден" end
        if target == ply then return false, "Нельзя перевести себе" end
        if not ply:CanAffordTransfer(amount) then return false, "Недостаточно средств" end
        
        return true, {ply, target, amount}
    end,
    execute = function(ply, target, amount)
