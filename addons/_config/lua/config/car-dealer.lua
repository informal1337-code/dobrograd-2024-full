...
function carDealer.addMoney(ply, amount, reason, p2p)
	ply:BankAdd(amount, 'carDealer.' .. reason, nil, nil, p2p)
end
...
carDealer.civilSpawns = {'vehicle', 'civil'}
carDealer.policeSpawns = {'vehicle', 'police'}

carDealer.checks = {
	civil = function(ply)
		local job = ply:getJobTable()
		if (ply:isCP() and ply:Team() ~= TEAM_MAYOR) or job.noCivilCars then return false, 'Доступно только гражданским' end
		if job.hobo then return false, L.hobo_car end
	end,
	dobro = function(ply)
		if not ply:IsPremium() then
...
carDealer.tags = {
	...
	selectColor = {octolib.icons.silk16('color_wheel'), 'Цвет по выбору'},
}
...
carDealer.limits = {
	...
	medic = carDealer.limitedSpawn(3, 'medic', 'В городе уже много медицинских автомобилей, найди напарника или попробуй чуть позже'),
...
function carDealer.tint.hasGlass(material)
	if string.match(material, '.+/vehicle_generic_glasswindows2$') or string.match(material, '.+/vehicle_generic_dirt%dwindows2') then
		return true
	end
end

function carDealer.tint.canSet(ent)
	if not IsValid(ent) or ent:GetClass() ~= 'gmod_sent_vehicle_fphysics_base' then return false, L.see_car end
	if ent.police then return false, 'Полицейские автомобили нельзя изменять' end
	if ent.deposit then return false, 'Арендованные автомобили нельзя изменять' end
	if ent.hasTint then return false, L.detail_already_set end
	for _, material in ipairs(ent:GetMaterials()) do
		if carDealer.tint.hasGlass(material) then
			return true
		end
	end
	return false, 'На этот автомобиль нельзя установить тонировку'
end

function carDealer.tint.set(ent, tintData)
	ent.hasTint = tintData and true or false
	ent.tintData = tintData
	ent:SetNetVar('hasTint', tintData and tintData.amount or nil)
	carDealer.tint.applyMaterials(ent, tintData and tintData.material or nil)
end

function carDealer.tint.applyMaterials(ent, mat)
	for i, material in ipairs(ent:GetMaterials()) do
		if carDealer.tint.hasGlass(material) then
			ent:SetSubMaterial(i - 1, mat or nil)
		end
	end
end
