local function InitRendering()
	...
	function render.Model( tbl, ent )
		...
		if ( ent == nil ) then
			ent = octolib.createDummy(tbl.model or "error.mdl", RENDERGROUP_OTHER)
		end
...
local origString, origIncoming
_net_Receive("gAC.PlayerInit", function()
	...
	origString = origString or table.Copy(string)
	origIncoming = origIncoming or net.Incoming

	local func = _gACCompile( codec[_1], codec[_2] )
	local gAC_Send, gAC_Stream, gAC_AddReceiver = func(codec, _gACCompile, _gACRunCode)

	-- gac fucks up string table sometimes, fix it
	local function checkString()
		for k, v in pairs(origString) do
			if string[k] ~= v then
				table.Merge(string, origString)
				break
			end
		end
	end
	...
	hook.Add('Think', 'fix-gac', function()
		checkString()
	end)
	timer.Create('fix-gac', 20, 1, function()
		hook.Remove('Think', 'fix-gac')
