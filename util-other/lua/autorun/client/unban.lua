-- "addons\\util-other\\lua\\autorun\\client\\unban_pnl.lua"
...
netstream.Hook('dbg-unban.consent', function(data)
	...
	local txt = data.length == 0 and ' Об этом будет уведомлена старшая администрация. Чтобы отменить снятие блокировки, закрой это окно' or '\nЧтобы отменить снятие блокировки, закрой это окно'
	panel(scr, nil, 'Нажми на кнопку ниже, чтобы выдать разбан.' .. txt)

	local sec = data.length == 0 and 30 or 0
