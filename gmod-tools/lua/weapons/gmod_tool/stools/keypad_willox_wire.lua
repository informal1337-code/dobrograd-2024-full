...
if CLIENT then
	language.Add("tool.keypad_willox_wire.name", "Кейпады - Wire")
	language.Add("tool.keypad_willox_wire.left", "Создать новый кейпад")
	language.Add("tool.keypad_willox_wire.right", "Обновить существующий")
	language.Add("tool.keypad_willox_wire.desc", "Создает кодовые замки")
...
function TOOL:RightClick(tr)
	...
	if trace_ent.KeypadData and trace_ent.KeypadData.Owner == ply:SteamID() then
...
if CLIENT then
	...
	function TOOL.BuildCPanel(CPanel)
		if not WireLib then
			CPanel:Help("Для этого тула необходим Wiremod")
			CPanel:Help("http://wiremod.com/")
			CPanel:Help("Аддон в воркшопе: #160250458")
		else
			local r, l = CPanel:TextEntry("Код доступа", "keypad_willox_wire_password")
			r:SetTall(22)

			CPanel:ControlHelp("Максимальная длина: 4\nРазрешены цифры 1-9")

			CPanel:CheckBox("Не показывать код на экране", "keypad_willox_wire_secure")
			CPanel:CheckBox("Приварить", "keypad_willox_wire_weld")
			CPanel:CheckBox("Закрепить", "keypad_willox_wire_freeze")
			...
			local granted = vgui.Create("DForm")
				...
				granted:NumSlider("Длина [1]", "keypad_willox_wire_length_granted", 0.1, 10, 2)
				granted:NumSlider("Перв. задерж. [2]", "keypad_willox_wire_init_delay_granted", 0, 10, 2)
				granted:NumSlider("Доп. задерж. [3]", "keypad_willox_wire_delay_granted", 0, 10, 2)
				granted:NumSlider("Доп. активаций [4]", "keypad_willox_wire_repeats_granted", 0, 5, 0)
			CPanel:AddItem(granted)

			local denied = vgui.Create("DForm")
				denied:SetName("Поведение при неправильном коде")

				denied:NumSlider("Длина [1]", "keypad_willox_wire_length_denied", 0.1, 10, 2)
				denied:NumSlider("Перв. задерж. [2]", "keypad_willox_wire_init_delay_denied", 0, 10, 2)
				denied:NumSlider("Доп. задерж. [3]", "keypad_willox_wire_delay_denied", 0, 10, 2)
				denied:NumSlider("Доп. активаций [4]", "keypad_willox_wire_repeats_denied", 0, 5, 0)
			CPanel:AddItem(denied)

			CPanel:Button("Настройки по умолчанию", "keypad_willox_wire_reset")
			...
			CPanel:Help("[1] Сколько времени подавать сигнал?")
			CPanel:Help("[2] Через сколько времени после активации подать сигнал первый раз?")
			CPanel:Help("[3] Через сколько времени после предыдущей подачи сигнала подать его еще раз?")
			CPanel:Help("[4] Сколько раз дополнительно нужно подать сигнал? ([3] - задержка между подачами)")
